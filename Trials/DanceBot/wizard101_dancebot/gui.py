...
class Configure(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        tk.Button(
            frame_bottomleft,
            text="Save",
            command=shared.set_save_settings,
            relief="solid",
            fg=self.fg_color,
    ...
    def frame_locations(self, frame: tk.Frame) -> None:
        ...
        tk.Label(
            frame,
            text="Locations to Farm",
            fg=self.fg_color,
            bg=self.bg_color,
            font=font.Font(size=9, underline=True),
        ...
        location_values = (
            Globals.settings["locations"]
            if self.valid_settings
            else (
                Configure.configure_settings.locations
                if Configure.configure_settings is not None
                else [None] * len(cities)
            )
        )
    ...
    def frame_snacks(self, frame: tk.Frame) -> None:
        ...
        tk.Label(
            frame,
            text="Pet Snacks to Feed",
            fg=self.fg_color,
            bg=self.bg_color,
            font=font.Font(size=9, underline=True),
        ).pack(padx=10, pady=(10, 0), anchor="e")
        snacks_names = [f"Snack {i}" for i in range(1, 6)]

        # three way nested ternary
        snack_values = (
            Globals.settings["snacks"]
            if self.valid_settings
            else (
                Configure.configure_settings.snacks
                if Configure.configure_settings is not None
                else [None] * len(snacks_names)
            )
        )
        ...
        for idx, (snack, snack_checked) in enumerate(snacks):
            ...
            self.create_checkbox(
                frame, anchor="center", text=snack, var=self.snack_boxes[idx]
            )
    ...
    def configure_games(self, frame: tk.Frame) -> None:
        ...
        # three way nested ternary
        num_games_text = (
            Globals.settings["num_games"]
            if self.valid_settings
            else (
                None
                if Configure.configure_settings is None
                or Configure.configure.settings.num_games == 1
                else Configure.configure.num_games
            )
        )
    ...
    def configure_resolutions(self, frame: tk.Frame) -> None:
        ...
        self.resolutions = ttk.Combobox(
            frame,
            value=available_resolutions,
            width=16,
            state="readonly",
        ...
        # three way nested ternary
        # use default settings or previously entered settings or previously saved settings
        self.resolutions.current(
            available_resolutions.index(
                Globals.settings["resolution"]
                if self.valid_settings
                else (
    ...
    def create_checkbox(
        self, master: tk.Frame, *, anchor: str, text: str, var: tk.IntVar
    ) -> None:
        tk.Checkbutton(
            master, text=text, variable=var, pady=0, fg=self.fg_color, bg=self.bg_color
        ).pack(padx=10, anchor=anchor)
...
class Playing(tk.Tk):
    def __init__(self, resolution: str, num_games: int, current_game: int) -> None:
        ...
        games_progress = ttk.Label(
            frame_bottom,
            text=f"Game {current_game + 1} of {num_games}",
