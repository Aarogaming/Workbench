...
class ConversationManager:
    ...
    def add_agent(self, agent_id: str, name: str, role: AgentRole) -> Dict[str, Any]:
        """Add agent to conversation"""
        if agent_id in self.agents:
            return {"error": f"Agent {agent_id} already exists"}

        agent = Agent(agent_id=agent_id, name=name, role=role)
        self.agents[agent_id] = agent
        return {"success": True, "agent_id": agent_id, "role": role.value}
    ...
    def get_conversation_summary(self) -> Dict[str, Any]:
        ...
        message_stats = {"statement": 0, "query": 0, "response": 0, "control": 0}
    ...
    def route_message_to_agents(
        self, sender_id: str, content: str, target_roles: List[AgentRole]
    ) -> Dict[str, Any]:
