...
class ContextSummarizer:
    ...
    def _calculate_salience(self, items: List[ContextItem]) -> Dict[int, float]:
        """Calculate salience scores for context items"""
        salience = {}

        if self.config.salience_model == SalienceModel.RECENCY:
            max_time = max((item.timestamp for item in items), default=0)
    ...
    def _extract_summary(
        self, items: List[ContextItem], target_length: int
    ) -> List[ContextItem]:
        """Extract most salient items"""
        salience = self._calculate_salience(items)

        sorted_idx = sorted(salience.keys(), key=lambda x: salience[x], reverse=True)
    ...
    def _abstract_summary(self, items: List[ContextItem], target_length: int) -> str:
    ...
    def summarize(self, items: Optional[List[ContextItem]] = None) -> Dict[str, Any]:
        ...
        original_length = sum(len(item.content.split()) for item in context)
        target_length = max(
            self.config.min_length, int(original_length * self.config.compression_ratio)
        )
        ...
        if self.config.strategy == SummarizationStrategy.EXTRACTIVE:
            ...
            summary_text = self._abstract_summary(context, target_length)
            selected = context
        ...
        if self.config.include_metadata:
            result["sources"] = list(set(item.source for item in selected))
    ...
    def hierarchical_summarize(self) -> Dict[str, Any]:
        ...
        for item in self.context_history:
            level1.add_context(
                item.content, item.source, item.importance, item.metadata
            )
        ...
        return {
            ...
            "total_compression": (
                level1_summary["compression_ratio"] * level2_result["compression_ratio"]
            ),
        }
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        total_tokens = sum(len(item.content.split()) for item in self.context_history)
        return {
            "total_items": len(self.context_history),
            "total_tokens": total_tokens,
            "avg_importance": (
                sum(item.importance for item in self.context_history)
                / len(self.context_history)
            ),
            "sources": list(set(item.source for item in self.context_history)),
        }
