...
class VersionConstraint:
    ...
    def satisfies(self, version: Version) -> bool:
        ...
        if op == ">=":
            ...
            return (version.major == constraint_ver.major and
...
class PluginVersionManager:
    ...
    def check_compatibility(self, plugin_id: str, required_version: VersionConstraint) -> bool:
        """Check if installed version satisfies requirement"""
        if plugin_id not in self.registry:
            return False
        return required_version.satisfies(self.registry[plugin_id])
    
    def get_latest_compatible(self, plugin_id: str, versions: List[Version]) -> Optional[Version]:
        """Get latest version that's compatible"""
        compatible = [v for v in versions if v.is_compatible(self.registry.get(plugin_id, Version(0, 0, 0)))]
