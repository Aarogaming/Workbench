...
def restructure_assets(root_dir):
    ...
    if os.path.exists(eldamar_path):
        for file in os.listdir(eldamar_path):
            if 'LUTs' in file:
                shutil.move(os.path.join(eldamar_path, file), os.path.join(root_dir, 'Visuals', 'LUTs', file))
            elif 'Backgrounds' in file or 'Overlays' in file or 'Transitions' in file or 'Titles' in file:
                shutil.move(os.path.join(eldamar_path, file), os.path.join(root_dir, 'Audio', 'Overlays', file))
            elif 'Explosions' in file or 'Particles' in file or 'Impact' in file or 'Fire' in file:
                shutil.move(os.path.join(eldamar_path, file), os.path.join(root_dir, 'Audio', 'SFX', file))

    # Move Programs
    programs_path = os.path.join(root_dir, 'Audio', 'Programs')
    if os.path.exists(programs_path):
        for file in os.listdir(programs_path):
            shutil.move(os.path.join(programs_path, file), os.path.join(root_dir, 'Tools', 'Software', file))
